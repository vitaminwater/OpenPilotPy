...
#                print
    ...
    def _sendpacket(self, type, objId, data=None):
        ...
        if data != None:
            crc.addList(data)
            self._writeStream("".join(map(chr,data)))
        
        self._writeStream(chr(crc.read()))
        
        self.txLock.release()
        
    def _writeStream(self, bytes):
        print("Writing: " + ":".join("{:02x}".format(ord(c)) for c in bytes))
